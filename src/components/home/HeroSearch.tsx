      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-5">
            {/* Trust Pill */}
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white border border-brand-200 shadow-sm text-xs font-semibold text-brand-700">
            ...
            <div className="bg-white p-2 sm:p-2.5 rounded-2xl shadow-lg shadow-slate-200/70 border border-slate-200 max-w-xl">
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                <div
                  onClick={onOpenSearch}
                  className="flex-1 flex items-center gap-2.5 px-3 py-1.5 cursor-pointer rounded-xl hover:bg-slate-50 transition-colors"
                >
                ...
                <button
                  onClick={onOpenSearch}
                  className="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 active:bg-brand-800 text-white font-bold text-xs sm:text-sm shadow-md shadow-brand-500/25 transition-all flex items-center justify-center gap-2 shrink-0 active:scale-[0.98]"
                >
                  <span>Search</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-w-xl">
                {HERO_SHORTCUTS.map((item) => (
                  <div
                    key={item.name}
                    onClick={() => {
                      const matched = POPULAR_SERVICES.find(
                        (s) => s.slug === item.serviceSlug || s.categorySlug === item.serviceSlug
                      );
                      onOpenBooking(matched);
                    }}
          ...
          <div className="lg:col-span-5 relative">
            <div className="relative mx-auto max-w-md lg:max-w-none">
              ...
              <div className="relative rounded-3xl overflow-hidden shadow-xl border-4 border-white bg-slate-900 aspect-[4/3.8] max-h-[380px]">
                <img
                ...
                <div className="absolute bottom-3 left-3 right-3 p-3.5 rounded-2xl bg-white/15 backdrop-blur-md border border-white/20 text-white">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-[10px] uppercase font-bold tracking-wider text-amber-300">
                        Top Rated Specialist
                      </span>
                      <p className="text-xs sm:text-sm font-bold">HVAC & Master Electricians</p>
                      <p className="text-[11px] text-slate-200">100% Background-checked & Police Verified</p>
                    </div>
              ...
              <div className="absolute -top-3 left-0 sm:-left-3 bg-white p-2.5 sm:p-3 rounded-2xl shadow-lg border border-slate-100 flex items-center gap-2.5 animate-float">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center font-bold">
                  <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
                </div>
                <div>
                  <div className="flex items-center gap-1">
                    <span className="text-xs sm:text-sm font-black text-slate-900">4.89 / 5</span>
                    <span className="text-[9px] text-emerald-600 font-bold bg-emerald-50 px-1 rounded">Top 1%</span>
                  </div>
                  <p className="text-[9px] text-slate-500 font-medium">Over 1,50,000+ Reviews</p>
                </div>
              </div>

              {/* Floating Live Badge 2: 45-Min Arrival */}
              <div className="absolute -bottom-3 right-0 sm:right-2 bg-white p-2.5 sm:p-3 rounded-2xl shadow-lg border border-slate-100 flex items-center gap-2.5 animate-float" style={{ animationDelay: "2s" }}>
                ...
                <div>
                  ...
                  <p className="text-[9px] text-slate-500">Live GPS tracking & OTP</p>
                </div>
              </div>
            </div>
          </div>
