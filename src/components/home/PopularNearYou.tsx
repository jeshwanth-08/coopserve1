...
export default function PopularNearYou({
  ...
  const filtered = POPULAR_SERVICES.filter((s) => {
    if (activeFilter === "cleaning") return s.categorySlug === "cleaning";
    if (activeFilter === "ac") return s.categorySlug === "ac-appliances";
    if (activeFilter === "electric") return s.categorySlug === "electrical";
    if (activeFilter === "plumb") return s.categorySlug === "plumbing";
    return true;
  });
