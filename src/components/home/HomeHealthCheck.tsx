...
export default function HomeHealthCheck({ onOpenBooking }: HomeHealthCheckProps) {
  ...
  const [metrics, setMetrics] = useState<HealthMetric[]>([
    {
      ...
      detail: "Hard water scaling detected on aerators. High risk of drain trap backflow and valve dripping.",
    ...
    },
    {
      ...
      status: "good",
      statusText: "✓ Good",
      detail: "Jet pump coil wash done recently. Compressor amp draw and refrigerant pressure are normal.",
    ...
    },
    {
      ...
      status: "warning",
      statusText: "⚠ Needs attention",
      detail: "Washing machine drain pump and refrigerator condenser coils require scheduled checkup.",
      recommendedServiceId: "svc-6",
      icon: <Wrench className="w-5 h-5 text-amber-500" />,
      lastChecked: "90 days since last service",
    },
    {
      id: "cleaning",
      category: "Cleaning",
      ...
      status: "good",
      statusText: "✓ Good",
      detail: "Tile grout cleanliness is optimal and bathroom scale levels are safely managed.",
    ...
    },
    {
      ...
      status: "warning",
      statusText: "⚠ Needs attention",
      detail: "Wardrobe hinges loose and main door cylinder lock requires alignment lubrication.",
      recommendedServiceId: "svc-3",
      icon: <Hammer className="w-5 h-5 text-amber-500" />,
      lastChecked: "Checkup advised",
    },
  ]);
  ...
  const recommendedService =
    POPULAR_SERVICES.find(
      (s) =>
        s.id === selectedMetric.recommendedServiceId ||
        s.slug === selectedMetric.recommendedServiceId ||
        s.categorySlug === selectedMetric.recommendedServiceId
    ) ||
    (selectedMetric.id === "plumbing"
      ? POPULAR_SERVICES.find((s) => s.slug === "plumber" || s.id === "svc-2")
      : null) ||
    (selectedMetric.id === "electrical"
      ? POPULAR_SERVICES.find((s) => s.slug === "electrician" || s.id === "svc-1")
      : null) ||
    (selectedMetric.id === "ac"
