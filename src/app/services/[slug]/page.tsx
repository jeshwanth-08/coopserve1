...
export default function CategoryDetailPage() {
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col justify-between">
        ...
        <div className="max-w-xl mx-auto px-4 py-20 text-center space-y-4">
          ...
          <p className="text-xs text-slate-500">
            We couldn't find a category matching "{slug}". Explore our full range of 12 core disciplines below.
          </p>
          <Link
            href="/#categories"
  ...
  const categoryServices = POPULAR_SERVICES.filter(
    (s) => s.categorySlug === category.slug || s.category.toLowerCase().includes(category.title.toLowerCase())
  );
  ...
  const handleOpenBooking = (service?: ServiceItem) => {
    setActiveServiceForBooking(service || categoryServices[0] || POPULAR_SERVICES[0]);
